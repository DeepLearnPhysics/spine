...
class MixedDataset(BaseDataset):
    ...
    The child roles are deliberately storage-independent: ``primary`` owns
    iteration and truth inputs, while ``cache`` supplies aligned materialized
    products. The primary selects its concrete dataset implementation through
    ``name``; the cache role implies the manifest-backed cache dataset.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        ----------
        primary : dict
            Authoritative dataset configuration. This source owns iteration
            order and generally supplies raw inputs or truth products.
        cache : dict
            Aligned materialized-product dataset configuration.
        dtype : str, optional
        ...
        cache_align_keys : dict, optional
            Mapping from primary alignment keys to cache alignment keys.
            If not provided, the dataset uses `source_<key>` when that key is
            present in the cache sample, and otherwise falls back to `<key>`.
        cache_key_map : dict, optional
            Rename map applied to cache product keys before merging.
        ...
        **kwargs : Any
        ...
        self.cache_align_keys = dict(cache_align_keys or {})
        self.cache_key_map = dict(cache_key_map or {})
