...
from .base import adjusted_mutual_info_score, adjusted_rand_score
...
def sbd(truth, pred, batch_ids=None):
    ...
    # ``unique_labels`` returns dense inverse labels plus the original unique
    # values. Best-Dice operates on the dense labels, so its cluster IDs are
    # the contiguous inverse-label range rather than the original values.
