...
def get_class_weights(labels, num_classes, mode="const", per_class=True):
    ...
    if is_numpy:
        ...
        def ones(size):
            return torch.ones(size, dtype=labels.dtype, device=labels.device)

        def empty(size):
            return torch.empty(size, dtype=torch.float, device=labels.device)
