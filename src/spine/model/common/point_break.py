        ...
        for label in np.unique(labels):
            ...
            if len(point_ids) > 2:
                ...
                for i, e in enumerate(mst_edges):
                    start_id, end_id = point_ids[e]
                    paths[i].append(end_id)
                    while end_id != start_id:
                        end_id = predecessors[start_id, end_id]
                        paths[i].append(end_id)
                ...
                for i in range(len(mst_tort)):
                    ...
                    # Preserve paths whose removal would reduce voxel reachability.
                    if reach > least_r:  # pragma: no cover - graph-geometry guard
