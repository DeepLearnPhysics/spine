...
from spine.model.layer.cluster.loss.misc import LovaszHingeLoss, iou_batch


class LinearSumAssignmentLoss(torch.nn.Module):
...
@torch.jit.script
def get_instance_masks(cluster_label: torch.LongTensor, max_num_instances: int = -1):
    ...
    instance_masks = torch.zeros((cluster_label.shape[0], max_num_instances)).to(
        device=cluster_label.device, dtype=torch.bool
    )
...
@torch.jit.script
def get_instance_masks_from_queries(
    cluster_label: torch.LongTensor, query_index: torch.Tensor
):
