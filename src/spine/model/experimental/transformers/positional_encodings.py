...
def get_normalized_coordinates(coords, spatial_size):
    ...
    normalized_coords = (coords[:, :3].float() - spatial_size / 2) / (spatial_size / 2)
...
class FourierEmbeddings(nn.Module):
    ...
    def normalize_coordinates(self, coords):
        if len(coords.shape) == 2:
            ...
            normalized_coords = (
                coords[:, :, : self.D].float() - self.spatial_size / 2
            ) / (self.spatial_size / 2)
            return normalized_coords
        else:
            raise ValueError(
                "Normalize coordinates saw {}D tensor!".format(len(coords.shape))
            )
