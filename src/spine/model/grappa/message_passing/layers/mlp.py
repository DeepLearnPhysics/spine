...
class MLPEdgeLayer(torch.nn.Module):
...
class MLPNodeLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        aggr_mlp : dict
            Configuration of the message aggregation MLP
        ...
        if self.attention:
            self.gate = torch.nn.Linear(message_width, 1)
        else:
            self.gate = None
    ...
    def forward(
        ...
        # If requested, make the network learn to explicitly tune down
        ...
        if self.attention:
            ...
            # Apply the gate weights to the message features
        ...
        # Aggregate the message with the node features, pass through MLP
...
class MLPGlobalLayer(torch.nn.Module):
    ...
    with the graph itself (N_g) to form a feature vector of dimension
    ...
    def forward(
        ...
        input_feats = all_node_feats
        if glob_feats is not None:
            input_feats = torch.cat((input_feats, glob_feats), dim=1)
