...
def check_track_orientation_ppn(
    ...
    """Determine track orientation from PPN endpoint predictions.

    The two proposed track endpoints are associated with their closest PPN
    candidates. The endpoint score then determines which proposal is the
    likely start of the track.

    Parameters
    ----------
    start_point, end_point : np.ndarray
        Proposed ``(3,)`` start and end coordinates.
    ppn_candidates : TensorData
        PPN candidates with an ``endpoint_scores`` feature.

    Returns
    -------
    bool
        ``True`` when the supplied orientation should be retained.
    """
    ...
    return bool(end_scores[0] < end_scores[1])
...
def get_track_length(
    ...
    if method in ["step", "step_next", "bin_pca"]:
        ...
        seg_lengths = get_track_segments(
            coordinates, segment_length, point, method, anchor_point, min_count
        )[-1]
        ...
        raise ValueError(f"Track length estimation method not recognized: `{method}`.")


@nb.njit(cache=True)
def check_track_orientation(
    coordinates: nb.float32[:, :],
    values: nb.float32[:],
    ...
    if method == "local":
        ...
        grad_start = get_track_deposition_gradient(
            coordinates,
            values,
            start_point,
            segment_length,
        ...
        grad_end = get_track_deposition_gradient(
            coordinates,
            values,
            end_point,
            segment_length,
        ...
        gradient = (grad_start - grad_end) / 2.0

        return bool(gradient >= 0.0)
...
@nb.njit(cache=True)
def get_track_deposition_gradient(
    coordinates: nb.float32[:, :],
    values: nb.float32[:],
    start_point: nb.float32[:],
    segment_length: nb.float32 = 5.0,
    ...
    if not len(valid_index):
        return 0.0, seg_dedxs, seg_rrs, seg_lengths
    ...
    gradient = cov[0, 1] / cov[0, 0] if cov[0, 0] > 0.0 else 0.0

    return gradient, seg_dedxs, seg_rrs, seg_lengths


@nb.njit(cache=True)
def get_track_segment_dedxs(
    coordinates: nb.float32[:, :],
    values: nb.float32[:],
    ...
    seg_clusts, seg_dirs, seg_lengths = get_track_segments(
        coordinates, segment_length, end_point, method, anchor_point, min_count
    )
    ...
    for i, seg in enumerate(seg_clusts):
        ...
        dx = seg_lengths[i]
        if len(seg) >= min_count and dx > 0.0:
        ...
        seg_rrs[i] = residual_range + dx / 2.0
        residual_range += dx
...
def get_track_segments(
    ...
    anchor_point: bool = True,
    min_count: int = 10,
) -> (nb.types.List(nb.int64[:]), nb.float32[:, :], nb.float64[:]):
    """Given a set of point coordinates associated with a track and one of its
    ...
    if method == "step" or method == "step_next":
        ...
        while len(left_index):
            ...
            dists = sm.distance.cdist(
                np.atleast_2d(seg_start), coordinates[left_index]
            ).flatten()
            ...
            if len(fail_index):
                ...
                if np.dot(next_closest - seg_start, seg_start - start_point) < 0.0:
                    fail_index = fail_index[:0]
                    if not len(pass_index):
                        break
            ...
            if (
                ...
                and np.max(dists[pass_index]) > 0.0
            ):
            ...
            if np.linalg.norm(direction):
                ...
                direction = np.array([1.0, 0.0, 0.0], dtype=coordinates.dtype)
        ...
        if point is not None:
            boundaries = np.arange(
                min(pstart, np.min(pcoordinates)), np.max(pcoordinates), segment_length
            )
        else:
            boundaries = np.arange(
                np.min(pcoordinates), np.max(pcoordinates), segment_length
            )

        seg_labels = np.digitize(pcoordinates, boundaries)
        seg_clusts = nb.typed.List.empty_list(np.empty(0, dtype=np.int64))
        for segment_id in range(len(boundaries)):
            seg_clusts.append(np.where(seg_labels == segment_id + 1)[0])

        # Compute the segment directions and lengths
        seg_dirs = np.empty(
            (len(seg_clusts), coordinates.shape[1]), dtype=coordinates.dtype
        )
