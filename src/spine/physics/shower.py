...
def sample_shower_points(
    ...
    longitudinal = t * LAR_X0

    # Transverse parameters (vectorized)
    R_core, R_tail, p_core = shower_transverse_params_gp(longitudinal, energy)
...
def shower_energy_density_3d(
    ...
    for i in range(n):
        ...
        longitudinal = dx * dir_unit[0] + dy * dir_unit[1] + dz * dir_unit[2]

        # Transverse distance to shower axis
        d2 = dx * dx + dy * dy + dz * dz
        r2 = d2 - longitudinal * longitudinal
        # Roundoff can make the squared transverse radius microscopically negative.
        if r2 < 0.0:  # pragma: no cover - floating-point precision guard
            r2 = 0.0
        r = np.sqrt(r2)

        g = shower_long_profile(longitudinal, energy, use_gp=use_gp)
        f = shower_trans_profile(r, longitudinal, energy)
...
def shower_transverse_params_gp(
    ...
    r_core_rm = z1 + z2 * tau
    # Very deep samples can make the GP tail exponent overflow even though the
    # corresponding radius is already effectively outside any detector volume.
    tail_exp_1 = np.minimum(k3 * (tau - k2), 80.0)
    tail_exp_2 = np.minimum(k4 * (tau - k2), 80.0)
    r_tail_rm = k1 * (np.exp(tail_exp_1) + np.exp(tail_exp_2))
