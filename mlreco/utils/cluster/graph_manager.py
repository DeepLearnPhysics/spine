...
class ClusterGraphConstructor:
    ...
    def prune_labels(self, labels : TensorBatch, skip_classes=[]):
        if len(skip_classes) == 0:
            ...
            out = TensorBatch(tensor, batch_size=labels.batch_size, has_batch_col=True)
            return out
    ...
    def initialize(self, 
        ...
        edge_batch_ids = TensorBatch(edge_batch_ids, batch_size=batch_size, has_batch_col=True)
        ...
        graph_state = {
            ...
            'gs_edge_index': TensorBatch(edge_index.T, counts=edge_batch_ids.counts),
            'gs_edge_shape': TensorBatch(edge_shape.view(-1, 1), counts=edge_batch_ids.counts),
            'gs_edge_attr': TensorBatch(edge_attr.view(-1, 1), counts=edge_batch_ids.counts),
            'gs_edge_label': TensorBatch(edge_truth.view(-1, 1), counts=edge_batch_ids.counts),
            'gs_edge_pred': TensorBatch(edge_pred.view(-1, 1), counts=edge_batch_ids.counts),
            'gs_edge_prob': TensorBatch(edge_prob.view(-1, 1), counts=edge_batch_ids.counts),
        }
