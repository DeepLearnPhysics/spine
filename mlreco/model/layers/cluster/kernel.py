...
from mlreco.model.layers.common.mlp import MLP
...
class DefaultKernel(nn.Module):
    ...
    def compute_edge_weight(self, sp_emb1, sp_emb2, ft_emb1, ft_emb2,
                            cov1, cov2, occ1, occ2):
        ...
        r = torch.max((r2 + self.eps)/(r1 + self.eps),
                      (r1 + self.eps)/(r2 + self.eps))
        pvec /= r
    ...
    def forward(self, x1, x2):
        ...
        assert x1.shape[1] == x2.shape[1] == 6 + self.num_features, (
...
class MixedKernel(DefaultKernel):
    ...
    def compute_edge_weight_coord(self, coord1, coord2, tan1, tan2,
                                  coord_cov1, coord_cov2, tan_cov1, tan_cov2):
    ...
    def forward(self, x1, x2):
        ...
        """
        # Decompose the two feature sets into their consituents
        assert x1.shape[1] == x2.shape[1] == 16 + self.num_features, (
                "The combined feature vector is not of the expected shape.")
        ...
        weight2 = self.compute_edge_weight_coord(
                coord1, coord2, tan1, tan2, coord_cov1, coord_cov2,
                tan_cov1, tan_cov2)
...
class BilinearKernel(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        assert x1.shape[1] == x2.shape[1] == self.num_features, (
...
class MLPKernel(nn.Module):
    ...
    def __init__(self, num_features, bias=False, mlp=None):
        ...
        if mlp is None:
            mlp = {'depth': 2, 'width': 32, 'activation': 'elu',
