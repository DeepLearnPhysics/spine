...
class MLPNodeLayer(nn.Module):
    ...
    def __init__(self, node_in, edge_in, glob_in, message_mlp,
                 aggr_mlp, reduction='mean', attention=False):
    ...
    def forward(self, node_feats, edge_index, edge_feats, glob_feats, batch):
        ...
        message_feats = scatter(
                message_feats, dest, dim=0,
                dim_size=node_feats.size(0), reduce=self.reduction)
...
class MLPGlobalLayer(nn.Module):
    ...
    def forward(self, node_feats, edge_index, edge_feats, glob_feats, batch):
        ...
        all_node_feats = scatter(
                node_feats, batch, dim=0, reduce=self.reduction)
