...
from mlreco.model.layers.factories import loss_fn_factory

from mlreco import TensorBatch, Meta
...
class NodeVertexLoss(torch.nn.Module):
    ...
    def forward(self, clust_label, clusts, node_pred, meta=None,
                start_points=None, end_points=None, **kwargs):
        ...
        **kwargs : dict, optional
        ...
        primary_ids = get_cluster_label_batch(
                clust_label, clusts, column=PRINT_COL)

        vertex_labels = np.empty((len(clusts.index_list), 3),
                                 dtype=primary_ids.dtype)
        for i, col in enumerate(VTX_COLS):
            vertex_labels[:, i] = get_cluster_label_batch(
                    clust_label, clusts, column=col).tensor
        ...
        if self.use_anchor_points:
            ...
            assert start_points is not None and end_points is not None, (
                    "Must provided particle end poins to anchor predictions.")

            # Get the particle end points, scale if necessary
            points = torch.hstack(
                    (start_points.tensor, end_points.tensor)).view(-1, 2, 3)
            if self.normalize_positions:
                ranges = (meta[0].upper - meta[0].lower)/meta[0].size
            ...
            vertex_pred = TensorBatch(
                    anchors + vertex_pred.tensor, vertex_pred.counts)
