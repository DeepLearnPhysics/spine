...
class TestRunInfoCreation:
    ...
    def test_runinfo_with_values(self):
        ...
        run_info = RunInfo(run=5426, subrun=127, event=8394)
    ...
    def test_runinfo_data_types(self):
        ...
        run_info = RunInfo(run=int(1000), subrun=int(50), event=int(12345))
...
class TestRunInfoTiming:
    ...
    def test_sequential_events(self):
        ...
        for event_num in range(10):
            run_info = RunInfo(run=base_run, subrun=base_subrun, event=event_num)
    ...
    def test_subrun_transitions(self):
        ...
        for subrun in range(5):
            for event in range(3):  # Just test first few events per subrun
                run_info = RunInfo(run=base_run, subrun=subrun, event=event)
        ...
        subruns = [
            ri.subrun for ri in run_infos[::3]
        ]  # Every 3rd (first of each subrun)
    ...
    def test_run_boundaries(self):
        ...
        # End of run scenario
        end_of_run = RunInfo(
            run=5426, subrun=999, event=9999  # Last subrun  # Last event
        )

        # Start of next run
        start_next_run = RunInfo(
            run=5427, subrun=0, event=0  # First subrun of new run  # First event
        )
...
class TestRunInfoIdentification:
    ...
    def test_unique_event_identification(self):
        ...
        for event_num in [1, 100, 1000, 5000, 9999]:
            run_info = RunInfo(run=5426, subrun=89, event=event_num)
    ...
    def test_event_matching(self):
        ...
        # TPC data
        tpc_event = RunInfo(
            run=common_run_info[0], subrun=common_run_info[1], event=common_run_info[2]
        )
...
class TestRunInfoDataQuality:
    ...
    def test_subrun_event_ranges(self):
        ...
        # Typical data ranges
        run_info = RunInfo(run=5426, subrun=89, event=1234)
    ...
    def test_data_quality_flags(self):
        ...
        # High quality event
        hq_event = RunInfo(run=5426, subrun=89, event=1234)
...
class TestRunInfoIntegration:
        ...
        for run in [5426, 5427, 5428]:
            for subrun in [89, 90]:
                for event in [0, 1000, 2000]:
                    run_info = RunInfo(run=run, subrun=subrun, event=event)
                    run_infos.append(run_info)
    ...
    def test_runinfo_edge_cases(self):
        ...
        # Maximum values
        max_run_info = RunInfo(run=999999, subrun=9999, event=999999)
        assert max_run_info.run == 999999
        assert max_run_info.subrun == 9999
        assert max_run_info.event == 999999

        # Minimum valid values
        min_run_info = RunInfo(run=1, subrun=0, event=0)
    ...
    def test_runinfo_database_style(self):
        ...
        for i in range(100):
            run_info = RunInfo(
                run=5426, subrun=i // 10, event=i % 10  # 10 events per subrun
            )
            events.append(run_info)
    ...
    def test_runinfo_string_representation(self):
        ...
        run_str = (
            f"Run {run_info.run}, Subrun {run_info.subrun}, Event {run_info.event}"
        )
