    ...
    def test_supernova_neutrino_burst(self):
        ...
        for i in range(5):
            ...
            sn_nu = Neutrino(
                ...
                current_type=1 if abs(pdg_codes[i]) == 12 else 2,  # CC for νe, NC for others
...
class TestNeutrinoIntegration:
    ...
    def test_realistic_detector_event(self):
        ...
        assert primary_muon.distance_travel > recoil_proton.distance_travel  # Muon penetrates
        assert primary_muon.energy_deposit < primary_muon.energy_init  # MIP exits
        assert abs(recoil_proton.energy_deposit - recoil_proton.energy_init) <= 50.0  # Proton stops
