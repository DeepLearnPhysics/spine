"""Static interface and documentation contracts for GNN model layers."""
    ...
    for path in gnn_sources():
        ...
        for node in ast.walk(tree):
            if not isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                continue
            if node.name not in {"forward", "generate", "get_base_features"}:
                continue
...
def test_gnn_local_variables_use_clear_snake_case_names():
    ...
    for path in gnn_sources():
        ...
        for function in ast.walk(tree):
            ...
            assert (
                not violations
            ), f"{path}:{function.lineno} uses unclear local names {violations}"


def test_gnn_public_interfaces_have_numpy_style_docstrings():
