"""Static interface and documentation contracts for common model layers."""
...
COMMON_DIR = (
    Path(__file__).resolve().parents[4] / "src" / "spine" / "model" / "layer" / "common"
)


def common_sources():
    """Yield all maintained common-layer source files."""
    yield from sorted(COMMON_DIR.glob("*.py"))
...
def test_common_code_does_not_use_ambiguous_batch_data_properties():
...
def test_common_data_path_methods_do_not_use_any_annotations():
...
def test_common_public_interfaces_have_numpy_style_docstrings():
...
def test_common_callable_parameters_are_documented():
    ...
    for path in common_sources():
        ...
        for node in public_definitions(tree):
            ...
            for section_name in ("Parameters", "Other Parameters"):
                ...
                stop = next(
                    (position - 2 for position in starts if position > start),
                    len(lines),
                )
