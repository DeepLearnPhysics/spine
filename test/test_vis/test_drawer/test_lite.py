...
from spine.vis.drawer.lite import (
    em_cone_trace,
    legend_trace,
    scatter_lite,
...
def test_lite_helpers_draw_tracks_showers_interactions_and_legends():
    ...
    assert track_line_trace(
        track.start_point, track.end_point, color=[1.0, 2.0]
    ).line.color == (
        1.0,
        2.0,
    )
...
def test_lite_validation_rejects_mismatched_colors():
    ...
    with pytest.raises(ValueError, match="line color"):
        track_line_trace(
            track.start_point, track.end_point, color=np.array([1.0, 2.0, 3.0])
        )
