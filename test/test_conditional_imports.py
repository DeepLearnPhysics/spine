...
class TestConditionalImports:
    ...
    def test_conditional_torch_availability(self):
        ...
        try:
            import torch
            assert TORCH_AVAILABLE is True, "TORCH_AVAILABLE should be True when torch is available"
        except ImportError:
            assert TORCH_AVAILABLE is False, "TORCH_AVAILABLE should be False when torch unavailable"
...
class TestNetworkXElimination:
    ...
    def test_no_networkx_imports(self):
        ...
        spine_modules = [name for name in sys.modules.keys() if name.startswith('spine')]
            ...
            if hasattr(module, '__file__') and module.__file__:
                ...
                try:
                    with open(module.__file__, 'r') as f:
                        content = f.read()
                        assert 'import networkx' not in content, f"{module_name} still imports networkx"
                        assert 'from networkx' not in content, f"{module_name} still imports from networkx"
    ...
    def test_torch_utilities_conditional(self):
        ...
        if TORCH_AVAILABLE:
            ...
            try:
                ...
                pytest.skip("Torch utilities not available despite TORCH_AVAILABLE=True")
        else:
