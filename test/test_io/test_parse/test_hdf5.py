...
def test_hdf5_index_list_parser_empty_with_scalar_count_event():
    """Empty cached index lists should still support explicit scalar counts."""
    parser = HDF5IndexListParser(
        dtype="float32",
        index_event="clusts",
        count_event="num_nodes",
...
def test_hdf5_edge_index_parser_rejects_non_2d_input():
    ...
    with pytest.raises(ValueError, match="must be 2D"):
        parser({"edge_index": np.asarray([0, 1, 2], dtype=np.int64), "num_nodes": 3})
