...
class Overlayer:
    ...
    def __init__(self, data_types, methods, multiplicity, mode='constant'):
        ...
        assert mode in self._modes, (
                f"Overlay mode not recognized: {mode}. Must be one of "
                f"{self._modes}.")
        self.mode = mode
    ...
    def get_assignments(self, batch_size):
        ...
        if self.mode == 'constant':
            ...
            if batch_size%self.multiplicity != 0:
                warn(f"The overlay multiplicity ({self.multiplicity}) is not a "
    ...
    def merge_scalars(self, batch, key, index):
        ...
        if self.methods[key] in ['first', 'match']:
            ...
            if self.methods[key] is None:
                raise ValueError(
                        f"Scalar overlay method not specified for {key}.")
    ...
    def merge_objects(self, batch, key, index):
        ...
        if self.methods[key] in ['first', 'match']:
            ...
            if self.methods[key] is None:
                raise ValueError(
                        f"Object overlay method not specified for {key}.")
