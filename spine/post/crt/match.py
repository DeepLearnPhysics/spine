...
class CRTMatcher:
    ...
    def __init__(self, driftv, match_method='threshold', tolerance=5,
                 detector=None, geometry_file=None, time_window=None,
                 min_part_size=None, min_crt_pe=None, match_distance=None):
    ...
    def get_matches(self, particles, crthits):
        ...
        for i, part in enumerate(particles):
            ...
            assert len(sources), (
                    "Cannot find CRT-TPC matches without `sources`.")
    ...
    def restrict_objects(self, particles, crthits):
        ...
        if self.min_part_size is not None:
            particles = [
                    part for part in particles if part.size > self.min_part_size]
