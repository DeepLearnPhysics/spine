...
class TriggerProcessor(PostBase):
    ...
    def __init__(self, file_path, correct_flash_times=True, flash_keys=None,
        ...
        if not os.path.isfile(file_path):
            raise FileNotFoundError(
                    f"Cannot find the trigger CSV file: {file_path}")
        ...
        if self.correct_flash_times:
            assert flash_keys is not None and len(flash_keys), (
    ...
    def process(self, data):
        ...
        trigger_mask = (
                (self.trigger_dict['run_number'] == run_id) &
        ...
        if self.correct_flash_times:
            ...
            offset = ((trigger.time_s - trigger.beam_time_s)*1e6
