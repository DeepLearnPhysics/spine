...
__all__ = ['LogRMSE', 'BerHuLoss', 'BinaryDiceLoss', 'BinaryLogDiceLoss',
           'BinaryMincutLoss', 'BinaryLogDiceCELoss',
           'BinaryLogDiceCEMincutLoss', 'FocalLoss']
...
class BinaryDiceLoss(nn.modules.loss._Loss):
    ...
    def forward(self, logits, targets):
        ...
        return 1. - (2*inter + self.eps)/(den + self.eps)
...
class BinaryLogDiceCELoss(nn.modules.loss._Loss):
    ...
    def __init__(self, log_dice=None, bce=None, reduction='mean',
                 w_dice=0.8, w_ce=0.2):
        """Initialize the loss function parameters.
...
class BinaryFocalLoss(nn.modules.loss._Loss):
    ...
    def __init__(self, alpha=1, gamma=2, logits=False, balance_loss=False,
                 reduction=True):
        """Initialize the loss function parameters.
