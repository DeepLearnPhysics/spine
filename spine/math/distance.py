...
from .base import mean, argmin

__all__ = ['cityblock', 'euclidean', 'sqeuclidean', 'minkowski', 'chebyshev',
           'pdist', 'cdist', 'farthest_pair', 'closest_pair']
...
@nb.njit(cache=True)
def get_metric_id(metric: nb.types.string,
                  p: nb.float32) -> nb.int64:
...
@nb.njit(cache=True)
def cityblock(x: nb.float32[:],
              y: nb.float32[:]) -> nb.float32:
...
def minkowski(x: nb.float32[:],
    ...
    float
        Minkowski distance
    """
    return pow(abs(y[0] - x[0])**p + abs(y[1] - x[1])**p + abs(y[2] - x[2])**p, 1./p)
...
@nb.njit(cache=True)
def _pdist_minkowski(x: nb.float32[:,:],
                     p: nb.float32) -> nb.float32[:,:]:
    res = np.empty((len(x), len(x)), dtype=x.dtype)
    for i in range(len(x)):
        res[i, i] = 0.
...
def cdist(x1: nb.float32[:,:],
    ...
    assert x1.shape[1] == 3 and x2.shape[1] == 3, (
            "Only supports 3D points for now.")
...
@nb.njit(cache=True)
def _cdist_cityblock(x1: nb.float32[:,:],
...
@nb.njit(cache=True)
def _cdist_euclidean(x1: nb.float32[:,:],
                     x2: nb.float32[:,:]) -> nb.float32[:,:]:
    res = np.empty((len(x1), len(x2)), dtype=x1.dtype)
    for i1 in range(len(x1)):
        for i2 in range(len(x2)):
...
@nb.njit(cache=True)
def _cdist_sqeuclidean(x1: nb.float32[:,:],
                       x2: nb.float32[:,:]) -> nb.float32[:,:]:
    res = np.empty((len(x1), len(x2)), dtype=x1.dtype)
    for i1 in range(len(x1)):
        for i2 in range(len(x2)):
...
@nb.njit(cache=True)
def _cdist_chebyshev(x1: nb.float32[:,:],
                     x2: nb.float32[:,:]) -> nb.float32[:,:]:
    res = np.empty((len(x1), len(x2)), dtype=x1.dtype)
    for i1 in range(len(x1)):
        for i2 in range(len(x2)):
...
@nb.njit(cache=True)
def _cdist_minkowski(x1: nb.float32[:,:],
                     x2: nb.float32[:,:],
                     p: nb.float32) -> nb.float32[:,:]:
    res = np.empty((len(x1), len(x2)), dtype=x1.dtype)
    for i1 in range(len(x1)):
...
def closest_pair(x1: nb.float32[:,:],
    ...
    if not iterative:
        ...
        while dist < tempdist:
            tempdist = dist
            dists = cdist(
                    xarr[set_id][idxs[set_id]][None, :], xarr[~set_id],
                    metric_id, p).flatten()
