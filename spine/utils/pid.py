...
from .globals import TRACK_SHP, MUON_PID, PION_PID, PROT_PID, KAON_PID, PID_MASSES
...
class TemplateParticleIdentifier:
    ...
    def __init__(self, use_table=True, use_mpv=False,
                 include_pids=(MUON_PID, PION_PID, PROT_PID, KAON_PID),
                 optimize_rr=False, max_rr=100, optimize_orient=True,
                 tracking_mode='step_next', **tracker):
    ...
    def __call__(self, points, depositions, end_point, start_point=None):
        ...
        chi2_per_pid = np.full(len(self.include_pids), -1.)
        for epoint in end_points:
            ...
            seg_dedxs, _, seg_rrs, _, _, _ = get_track_segment_dedxs(
                     points, depositions, epoint, method=self.tracking_mode,
                     **self.tracking_kwargs)

            # Only keep segments for which the dE/dx could be measured
            mask = np.where(seg_dedxs > 0.)[0]
            if len(mask) < 1:
                continue
            ...
            for i, pid in enumerate(self.include_pids):
                ...
                if chi2_per_pid[i] < 0. or chi2 < chi2_per_pid[i]:
                    chi2_per_pid[i] = chi2

        # Return
        if np.all(chi2_per_pid == -1.):
