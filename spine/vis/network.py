...
def network_topology(points, clusts, edge_index, clust_labels=None,
    ...
    **kwargs : dict, optional
    ...
    traces = scatter_clusters(
            points, clusts, color=clust_labels, single_trace=single_trace,
            name=node_name, mode=mode, **kwargs)
...
def network_schematic(clusts, edge_index, clust_labels, edge_labels=None,
                      color=None, name=None, linewidth=2, **kwargs):
    ...
    **kwargs : dict, optional
    ...
    assert len(clust_labels) == len(clusts), (
            "Must provide a primary label for each cluster.")
    assert np.all((clust_labels == 0) | (clust_labels == 1)), (
    ...
    node_trace = scatter_points(
            pos, color=clust_labels, hovertext=node_labels,
            hoverinfo=['x', 'y', 'text'], markersize=node_sizes,
            name=node_name, **kwargs)
