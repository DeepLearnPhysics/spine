...
class TrainDrawer:
    ...
    def __init__(self, log_dir, interactive=True, paper=False, alpha=0.5,
                 train_prefix='train', val_prefix='inference', separator=':'):
    ...
    def initialize_matplotlib(self, paper):
        ...
        if paper:
            ...
            sns.set(rc={'figure.figsize': (9,6)}, context='notebook',
                    font_scale=2)
            sns.set_style('white')
    ...
    def draw(self, model, metric, limits=None, model_name=None,
             metric_name=None, max_iter=None, step=None, smoothing=None,
             iter_per_epoch=None, print_min=False, print_max=False,
             same_plot=True, leg_ncols=1, figure_name=None):
            ...
            for j, key in enumerate(dfs.keys()):
                ...
                if smoothing is not None and smoothing > 1:
                    metric_t = metric_t.rolling(
                            smoothing, min_periods=1, center=True).mean()
                ...
                if not self.interactive:
                    ...
                    axis.plot(
                            epoch_t, metric_t, label=label, color=color,
                    ...
                    traces += [go.Scatter(
                        x=epoch_t, y=metric_t, name=label,
                        line={'color': color}, legendgroup=legendgroup,
                    ...
                    if draw_val[key]:
                        ...
                        traces += [go.Scatter(
                            ...
                            error_y_array=metric_v_err, mode='markers',
                            hovertext=hovertext, marker={'color': color},
        ...
        if not self.interactive:
            if not same_plot:
                ...
                plt.legend(ncol=leg_ncols, title=legend_title, bbox_to_anchor=(1.05, 1), loc='upper left')
